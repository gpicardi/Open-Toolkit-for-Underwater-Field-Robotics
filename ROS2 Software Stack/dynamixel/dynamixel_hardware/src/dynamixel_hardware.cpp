...
CallbackReturn DynamixelHardware::on_init(const hardware_interface::HardwareInfo & info)
{
  ...
  //transmissions patch
  reductions_.resize(info_.joints.size(), 1.0);
  offsets_.resize(info_.joints.size(), 0.0);
  for (uint i = 0; i < info_.joints.size(); i++) {
    // parse transmission data from info
    reductions_[i] = info_.transmissions[i].joints[0].mechanical_reduction;
    offsets_[i] = info_.transmissions[i].joints[0].offset;
  }
...
return_type DynamixelHardware::read(const rclcpp::Time & /* time */, const rclcpp::Duration & /* period */)
{
  ...
  for (uint i = 0; i < ids.size(); i++) {
    joints_[i].state.position = dynamixel_workbench_.convertValue2Radian(ids[i], positions[i])/reductions_[i] + offsets_[i]; // transmission patch
...
CallbackReturn DynamixelHardware::set_joint_positions()
{
  ...
  for (uint i = 0; i < ids.size(); i++) {
    ...
    commands[i] = dynamixel_workbench_.convertRadian2Value(
      ids[i], static_cast<float>(reductions_[i]*(joints_[i].command.position-offsets_[i]))); //transmission patch - use offset and reduction before writing
  }
